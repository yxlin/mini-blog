...
此文除了展示 data augumentation 的優點外，還展示[前文](https://yslin.me/blog/QMNIST)曾提到 ensemble 的概念。我們將使用三個深層[CNN模型](https://github.com/kkweon/mnist-competition)VggNet16, ResNet and VggNet5來進行交通號誌的分類作業。簡言的說 ensemble 模型指的是使用多個模型共同決定分類的推測，而多數決 (Majority voting) 是最常用的 ensemble 的實現方式之一。
...
3. 資料增強(Data augumentation)是一統計學術語借用，機器學習文獻指稱些微不同的概念。在知覺心理學中，雖然同一個外界物體的影像，在不同的觀察條件下，其物理影像呈現不同，然而觀察者心理上會將這些不同的物理影像感知、解釋為同一個物體，如果它確實是同一個物體。在機器學習中，資料增強指的是，我們使用軟體，例如Photoshop或是任何程式語的的影像操弄方法改變同一個影像檔的明暗、模糊程度或是改變環境的光照方式，使同張照片, 同一物體，外觀改變，例如，色彩，明暗或是擺放角度等等，但實際照片的內容不變。如此在進行訓練時，資料樣本總數會明顯地增加，如下圖不同裁切方式的限速20號誌。
