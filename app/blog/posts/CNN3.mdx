...
在此類簡易的分類作業中，非常簡單的CNN模型通常就可以達到非常好的正確率>80%，且可以分類不同的類型的圖檔，但是如果為了達到和人工判別，非常接近的一致性(如>95%的正確率)，模型勢必變得更為複雜。此例中雖然圖檔解析度較高，因此增加了記憶體的負擔，即使使用批次處理的方式，幫助仍然有限，特別是關鍵的GPU記憶體。下文我們將使用另一種方法Data augumentation，並將圖檔進行裁切，測視是否可以改善記憶體的問題，並同時使用更複雜的模型以達到更高的正確率。
