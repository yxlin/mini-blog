...
上面的例子顯示，在三次遞迴中分別取出[3, 1, 2, 7]，[1, 7, 1, 2]及[1, 2, 5, 4]，三個片段的輸入向量，因為stride 使設置為2，所以每次移動二個單位，而kernel向量有四個數值，所以我們取的片段長段為4。zero padding方法的目的之一就是協調kernel向量長度及輸入向量長度，以利進行向量內積運算。
...
CNN借由模仿生物的視神經機制介紹了許多新的概念，這此概念實現於數值演算中成為CNN的基石，除了以上的簡介外，實際計算時，演算法設計者還必須考量矩陣上數值的配置方法，以及計算的效率，幸運的是，上述介紹的常用機制在Keras及PyTorch套件中都有不同的實作函數可使用，在下一個文章中，我們將以實際的例子來討論如何使用CNN的進行影像辨識。