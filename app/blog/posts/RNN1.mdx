...
上面的矩陣將每一個時間點儲存為一個Python `list`，如[70, 65, 62, 46]。加上上例的歌曲有有192個時間點，所以它有192 個`list`，這些`list`被儲存在一個大的 `list` 之中。

因為tensorflow或是PyTorch等套件必須自由地在CPU或GPU，或者兩者上進行運算，加上CPU及GPU所能直接使用的記憶體是不同的硬體單元，Python的原生資料型態預設使用CPU及其記憶體來運算，所以我們必須將 `list` 的資料型態轉換為tensor，如此方能自由地在CPU及GPU的運算空間運作。下面用`train_chorales`作為範別說明。 
...
以下解釋一項稱為Sequence to Sequence的時序資料整理方法，此方法的指導原則是希望有較早時間點的一小段(例如5個時間點)資料作為特徵值，而此段資料所對應的目標值則是後移一小段時間，等長資料段落，且此資料段落有部份資料是和特徵值的資料有時間上的重疊。以下我們使用眾讚歌範例來說明。
...
5. 最後，我們將每一筆樣本用`tf.where`轉換。此函數如同一個`ifesle`語法，將數值0維持原樣，將其他數值減去35 (window - min_note + 1)，所以原來代表C1音符的36，改為1，而原來代表A5音符的81變為46。
...
7. 最後我們使用Dataset類別裡的 `batch`函數將，每兩筆樣本收集為一個批次處理單元，並將代表音符的1到23個數值做為樣本，`[:, :-1]`代表除了最後一欄的所有的欄位)，以上面的 0 tf.Tensor為例，即，
```
# 第一批次中的第一筆樣本
[39 35 30 23 39 35 30 23 39 35 30 23 39 35 30 23 40 35 23 20 40 35 23]
```

並將第二個數值至最後一欄的數值為目標，`[:, 1:]`代表從第二欄開始至最後一欄的。因為Python以0開始計次，所以1代表第二個數值。加上之前的`tf.reshape(window, [-1])`函數已經將每一個樣本整理為行向量，所以每一筆樣本只有一行。
...
# 將每一筆樣本，整理為(樣本，目標)的資料集
chorales= chorales.map(create_target)

# 0 tf.Tensor(
# [[39 35 30 23 39 35 30 23 39 35 30 23 39 35 30 23 40 35 23 20 40 35 23]
#  [39 35 30 23 40 35 23 20 40 35 23 20 40 35 25 20 40 35 25 20 42 34 27]], shape=(2, 23), dtype=int32) tf.Tensor(
# [[35 30 23 39 35 30 23 39 35 30 23 39 35 30 23 40 35 23 20 40 35 23 20]
...
#  [42 34 27 15 42 34 27 15 42 34 27 15 42 35 27 20 42 35 27 20 42 34 27]], shape=(2, 23), dtype=int32) tf.Tensor(
