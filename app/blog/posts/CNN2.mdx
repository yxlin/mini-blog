---
title: '使用卷積類神經網絡辨識簡單影像'
publishedAt: '2024-08-29'
summary: 'The post uses MNIST to demonstrate how one may use the CNN to recognize the hand-written arabic numerals.'
---

Convolution 在中文翻譯中，有稱為，捲積、疊積、褶積、旋積、或卷積，也許是據其操作方法-輸入向量如同卷軸般的分段移動，而後再將這些小段與Kernel向量做內積運算-所做的翻譯，所以CNN有時被譯為「卷積(類)神經網絡」，因為CNN的字元數較少，我們偏好使用CNN，但當語意需要時，有時仍會使用「卷積(類)神經網絡」。
...
下一步，我們使用上段程式碼處理後的資料，分別訓練一組MLP模型及一組CNN模型。兩組程序都只使用30次遞迴。
...
雖然使用了較少的參數，CNN表現比MLP高了約3.5百分點，相當於，在10,000張圖片中，CNN誤認了233圖片，而MLP誤認了近555張圖片。
...
在面對類似的影像辨識作業時CNN所需要最佳化的參數總量小於相近的MLP模型，因此當所需分析影像愈來愈複雜，單一影像檔所輸入的特徵值必然增多，因此在可用的方法及工具不變的情況下，若要達到類似的高辨識率，我們必須設置更多層的類神經網絡，且每一層網絡必然會有更多的節點，因此採用CNN成為較經濟的方法。

就算一個簡單的CNN模型已經表現得不錯，但在某些要求極高的應用場景，例如銀行匯兌，即使只有不到3%的錯誤率，也可能造成巨大的損失。為了達到更高的準確度，我們可以增加CNN模型的複雜度，或者使用Dropout這種技巧來降低過擬合的風險。
